...
PCMDataSource::PCMDataSource(QAudioFormat audio_format, qint64 threshold_ms, QObject *parent) 
    : QIODevice(parent), m_audio_format(audio_format), m_size(0), m_pos(0), m_threshold_ms(threshold_ms) {
  open(QIODevice::ReadWrite);
  // 10 seconds
  int max_size = audio_format.sampleRate() * audio_format.channelCount() *
               audio_format.bytesPerSample() * qMax(qint64(DEFAULT_THRESHOLD_SECONDS), threshold_ms / 1000);
...
qint64 PCMDataSource::writeData(const char *data, qint64 len) {
  ...
  // 检查缓冲区状态和边界
  if (m_buffer.isEmpty() || m_size < 0 || m_pos < 0 || m_size > m_buffer.size() || m_pos > m_size) {
    // 重置缓冲区状态
    clear();
  }
  ...
  if (len > available_space) {
    ...
    if (m_pos > 0 && m_size > m_pos) {
      ...
      if (remaining_data > 0 && remaining_data <= m_buffer.size() && m_pos + remaining_data <= m_buffer.size()) {
...
qint64 PCMDataSource::available_ms() {
    ...
    qint64 bytes_per_second = m_audio_format.sampleRate() * m_audio_format.channelCount() * m_audio_format.bytesPerSample();
...
AudioPlay::AudioPlay(QAudioFormat audio_format, QObject *parent)
    : QObject(parent), m_audio_format(audio_format), m_volume(1.0), m_balance(0.0) {
  ...
  // 设置合理的缓冲区大小，避免卡顿和性能问题
  // 计算200ms的缓冲区大小
  int buffer_size = audio_format.sampleRate() * audio_format.channelCount() * 
                   audio_format.bytesPerSample() * 0.2; // 200ms
  m_audio_sink->setBufferSize(buffer_size);
  connect(m_pcm_data_source.get(), &PCMDataSource::signal_request_more_data, this,
          &AudioPlay::signal_request_more_data);
...
bool AudioPlay::is_playing() {
  return m_audio_sink->state() == QAudio::ActiveState || m_audio_sink->state() == QAudio::IdleState;
}
