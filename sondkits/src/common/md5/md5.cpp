...
#define SET(n)                                                                 \
  (ctx->block[(n)] =                                                           \
       (MD5_u32)ptr[(n) * 4] | ((MD5_u32)ptr[(n) * 4 + 1] << 8) |              \
       ((MD5_u32)ptr[(n) * 4 + 2] << 16) | ((MD5_u32)ptr[(n) * 4 + 3] << 24))
...
std::string md5(std::string dat) { return md5(dat.c_str(), dat.length()); }

/* Generate shorter md5sum by something like base62 instead of base16 or base10.
