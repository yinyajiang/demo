#include "decodedatasource.h"
#include <QDebug>
#include <QtGlobal>

DecodeDataSource::DecodeDataSource(int64_t frame_size,
                                   std::shared_ptr<DecodeQueue> decode_queue)
    : DataSource(frame_size), m_decode_queue(decode_queue) {}

int64_t DecodeDataSource::realReadData(uint8_t *data, int64_t maxlen) {
  if (!data || maxlen <= 0) {
    return 0;
  }
  return m_decode_queue->readData(data, maxlen);
}

bool DecodeDataSource::realIsEnd() const { return m_decode_queue->readEnded(); }

int64_t DecodeDataSource::bytesAvailable() const {
  return m_decode_queue->bytesAvailable();
}

void DecodeDataSource::realClear() { m_decode_queue->clear(); }
