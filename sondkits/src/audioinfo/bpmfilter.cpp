...
BPMFilter::BPMFilter(std::shared_ptr<AudioDecoder> audio_decoder)
    : AudioThroughFilter(BPM_FILTER_HOP_SIZE * sizeof(float), true), m_audio_decoder(audio_decoder) {
      assert(m_audio_decoder->targetChannels() == 1);
      assert(m_audio_decoder->targetSampleFormat() == AV_SAMPLE_FMT_FLT);

      m_tempo = new_aubio_tempo("default", BPM_FILTER_BUF_SIZE, BPM_FILTER_HOP_SIZE,audio_decoder->targetSampleRate());
