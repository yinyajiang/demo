...
void AudioPlayer::open(const std::filesystem::path &in_fpath) {
  m_audio_decoder = std::make_unique<AudioDecoder>(DEFAULT_SAMPLE_RATE, DEFAULT_CHANNELS, DEFAULT_SAMPLE_AV_FORMAT);
  ...
  connect(m_audio_play.get(), &AudioPlay::signal_request_more_data, this, &AudioPlayer::on_request_more_data);
}

void AudioPlayer::on_request_more_data() {
  if (!m_audio_decoder || !m_audio_play) {
    return;
  }

  // 减少一次解码的帧数，避免阻塞UI线程太久
  // 同时增加错误处理和内存管理
  for (int i = 0; i < 10 && !m_audio_decoder->is_end(); i++) {
    int data_size = 0;
